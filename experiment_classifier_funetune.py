...
class ClsModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        if conf.manipulate_mode in [ManipulateMode.chexpert_all, 
                                    ManipulateMode.chexpert_finetune,
                                    ManipulateMode.chexpert_train, 
    ...
    def load_dataset(self):
        if self.conf.manipulate_mode == ManipulateMode.chexpert_all:
            return ChexpertAttrDataset(data_paths['chexpert'],
                            self.conf.img_size,
                            do_augment=True)

        elif self.conf.manipulate_mode == ManipulateMode.chexpert_finetune:
            return ChexpertFinetune(data_paths['chexpert'],
                            self.conf.img_size,
                            do_augment=True) 
        
        elif self.conf.manipulate_mode == ManipulateMode.chexpert_train:
            return CXRAttrDataset(data_paths['chexpert_train'],
                                self.conf.img_size,
                                do_augment=True,
                                fold='train'
            )
        
        elif self.conf.manipulate_mode == ManipulateMode.mimic_all:
            return MimicAttrDataset(data_paths['mimic'],
                            self.conf.img_size,
                            do_augment=True)

        elif self.conf.manipulate_mode == ManipulateMode.mimic_finetune:
            return MimicFinetune(data_paths['mimic'],
                            self.conf.img_size,
                            do_augment=True)
        
        elif self.conf.manipulate_mode == ManipulateMode.padchest_train:
            return PadchestAttrDataset(data_paths['padchest_train'],
                            self.conf.img_size,
                            do_augment=True) 
